...
2. **Ideal comparison**, a raw score is meaningless without context. NDCG normalizes by the best possible score achievable given the available relevant documents, making scores comparable across queries with different numbers of relevant documents.
...
$CG@K = \sum rel(i), \space for\space i = 1\space to\space K$
...
$DCG@K = \sum\frac{rel(i)}{\log_2 (i+1)}$

The discount at each rank:

| Rank | Discount                             |
| ---- | ------------------------------------ |
| 1    | $\log_2 2$ = 1.000, no discount      |
| 2    | $\log_2 3$ = 1.585, divide by 1.585  |
| 3    | $\log_2 4$ = 2.000, divide by 2.000  |
| 4    | $\log_2 5$ = 2.322, divide by 2.322  |
| 5    | $\log_2 6$ = 2.585, divide by 2.585  |
| 10   | $\log_2 11$ = 3.459, divide by 3.459 |

The $\log_2$ discount grows slowly, rank 2 is penalized less than rank 3, which is
penalized less than rank 4. This reflects the diminishing marginal cost of lower positions, consistent with how users scan results.

Alternative DCG formula (used in some benchmarks, emphasizes highly relevant docs):

$DCG@K = \sum\frac{2^{rel(i) - 1}}{\log_2 (i+1)}, \space for\space i = 1\space to\space K$
...
$$
\begin{align}
DCG@5 &= \frac{2}{\log_2(2)} + \frac{3}{\log_2(3)} + \frac{0}{\log_2(4)} + \frac{1}{\log_2(5)} + \frac{0}{\log_2(6)} \\
...
$$
\begin{align}
IDCG@5 &= \frac{3}{\log_2(2)} + \frac{2}{\log_2(3)} + \frac{1}{\log_2(4)} + \frac{0}{\log_2(5)} + \frac{0}{\log_2(6)} \\
...
The $\log_2$ discount is standard but arbitrary. Different discount functions would
